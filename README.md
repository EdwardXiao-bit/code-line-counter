...
- **运行计时**：统计结束后显示本次运行用时（命令行与网页都支持）。
- **逐文件日志**：命令行模式每统计完一个文件就输出一行日志（语言、路径、code/blank/comment）。
...
示例输出（前面逐文件打印日志，末尾显示运行用时）：

```
[C++] src\main.cpp  code=120  blank=8  comment=15
[C++] src\counter.hpp  code=326  blank=37  comment=25
...
------------------------------------------------------------
